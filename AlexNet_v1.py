...
# Function to build the AlexNet_v1 model
def build_model():
    model = models.Sequential([
        layers.Conv2D(96, kernel_size=(11,11), strides=(4,4), activation='relu', input_shape=(*IMG_SIZE, 3)),
...
for fold_no, (train_indices, test_indices) in enumerate(kfold.split(inputs, targets), 1):
    ...
    metrics_logger = MetricsLogger(
        f"metrics_AlexNet_v1_fold_{fold_no}.log",
        X_val,
        y_val,
        fold_no,
        f"confusion_matrix_AlexNet_v1",
    )
    ...
    save_classification_report(
        ...
        y_pred,
        class_names,
        f"classification_report_AlexNet_v1_fold_{fold_no}.txt",
    )
