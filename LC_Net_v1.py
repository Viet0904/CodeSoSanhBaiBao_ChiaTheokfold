...
targets_one_hot = tf.keras.utils.to_categorical(targets, num_classes)

checkpoint = tf.keras.callbacks.ModelCheckpoint(
    "best_model_LC_Net_v1.keras",
...
for fold_no, (train_indices, test_indices) in enumerate(
    ...
    metrics_logger = MetricsLogger(
        f"metrics_LC_Net_v1_fold_{fold_no}.log",
        X_val,
        y_val,
        fold_no,
        f"confusion_matrix_LC_Net_v1",
    )
    ...
    save_classification_report(
        ...
        y_pred,
        class_names,
        f"classification_report_LC_Net_v1.txt",
    )
