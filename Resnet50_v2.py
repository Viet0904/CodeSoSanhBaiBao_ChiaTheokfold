...
def build_model():
    ...
    # Res-2a
    x = layers.Conv2D(256, (1, 1), padding='valid', activation='relu')(x)
    x = layers.Conv2D(256, (1, 1), padding='valid', activation='relu')(x)

    # Res-3a
    x = layers.Conv2D(512, (1, 1), padding='valid', activation='relu')(x)
    x = layers.Conv2D(512, (1, 1), padding='valid', activation='relu')(x)

    # Res-4a
    x = layers.Conv2D(1024, (1, 1), padding='valid', activation='relu')(x)
    x = layers.Conv2D(1024, (1, 1), padding='valid', activation='relu')(x)

    # Res-5a
    x = layers.Conv2D(2048, (1, 1), padding='valid', activation='relu')(x)
    x = layers.Conv2D(2048, (1, 1), padding='valid', activation='relu')(x)
...
checkpoint = ModelCheckpoint(
    "best_model_ResNet50_v2_tangcuong.keras",
...
for fold_no, (train_indices, test_indices) in enumerate(
    ...
    metrics_logger = MetricsLogger(
        f"metrics_ResNet50_v2_tangcuong_fold_{fold_no}.log",
        X_val,
        y_val,
        fold_no,
        f"confusion_matrix_ResNet50_v2_tangcuong",
    )
    ...
    save_classification_report(
        ...
        y_pred,
        class_names,
        f"classification_report_ResNet50_v2_tangcuong.txt",
    )
